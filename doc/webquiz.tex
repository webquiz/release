%-----------------------------------------------------------------------------
%  Copyright (C) 2004-2025 Andrew Mathas, University of Sydney
...
\begin{document}
  ...
  \subsection{What \WebQuiz does and does not do}
      ...
      By default, \WebQuiz quizzes are not timed, however, a time-limit
      can be imposed using the \LaTeXCode|timed| document class option.

      \WebQuiz was designed as revision tool for students, rather than
      as an assessment tool. In particular, out of the box, \WebQuiz
      does not provide a mechanism for recording the marks obtained by
      the students taking the quiz (\autoref{SS:classOptions}). In fact,
      it is possible to configure \WebQuiz to record marks, however,
      \WebQuiz does not attempt to solve the technicalities of student
      authentication or of ensuring that the quiz content is secure.
      Currently, even though the solutions to the quiz questions do not
      appear in the \HTML source code for the quiz pages it is possible
      to access the answers if you know what you are doing.
  ...
  \begin{itemize}[nosep]
     ...
     from \Ctan before you try and use \WebQuiz.
      ...
      \item \python[Python 3]. As of writing python 3.13 is available
  ...
  \begin{quote}
    \begin{description}
      ...
      \item[discussion] Includes (optional) discussion, or lesson,
  ...
  correct answer as a string, which it must match exactly.
    ...
    the following:
    \ScreenShot{A question with an answer}{answer}
    The general syntax of the \LaTeXCode|\answer| macro is:
    \begin{latexcode}
    ...
    \end{latexcode}
    with \LaTeXCode|string| being the default.  The \LaTeXCode|\answer*|
    commands works in exactly the sane as \LaTeXCode|\answer| except
    that it does not print the word ``Answer'' (or equivalent in other
    languages) before the input box.
    ...
    \ScreenShot{A question with a string answer}{answer-string}
  ...
  In addition to asking questions, it is possible to display lesson,
  ...
  links between them.  \LaTeX{} provides the \LaTeXCode|\label| and
  ...
  As a second example, we can create a ``Start quiz'' button that that
  open Question~1 on any quiz, using \LaTeXCode|\Qref[Start quiz]{1}|.
  (See also the \LaTeXCode|start| document class option in
  ...
  \subsubsection{Index pages for quizzes}\label{SS:index}
    ...
    \ScreenShot{Example breadcrumbs with drop-down menu}{breadcrumbs-dropdown}
    ...
    ``magic crumbs'' listed above, is allowed to contain text ---
  ...
  Finally, we note that certain special characters in URLs can cause
  problems. For example, \WebQuiz will give an error if you try to use
  \LaTeXCode|\BreadCrumbs{Home www.maths.usyd.edu.au/~mathas}| because
  \TeX4ht\ expands the $\sim$-character. The easiest fix for this is to
  add a \LaTeXCode|\string| command before any special characters in the
  breadcrumbs:

  \InputLatexCode{breadcrumbs-user}
  ...
  \subsection{\WebQuiz document class options}\label{SS:classOptions}
    ...
    \begin{latexcode}
      ...
      separatepages, showsidemenu, theme, timed, tikz
    \end{latexcode}
  ...
  \begin{description}
    ...
    \CrossIndex{document class options}{randomorder}
      ...
      which is the order that the question appear in the \LaTeX{}
    ...
    \item[feedback, nofeedback]
    \CrossIndex{document class options}{feedback}
    \CrossIndex{document class options}{nofeedback}
    ...
    path\footnote{To help \LaTeX/\WebQuiz find your language file you
    ...
    \item[timed]\CrossIndex{document class options}{timed}
    Use \LaTeXCode|timed=X| to set a maximum time limit in
    \textit{minutes} for completing the quiz. In practice, it is not
    clear how useful this feature is because the user may be able to
    reset the time through their browser.

    \InputLatexCode{timed}
    produces
    \ScreenShot{Randomly ordered questions}{timed}
  ...
  \begin{description}
      ...
      \begin{dangerous}
        ...
        document-class option then you should first check that the
...
\section{The \WebQuiz program} \index{usage}
      ...
      The following command-line options are mainly useful for
      code developers.
  ...
  \subsubsection*{Institution settings}
    ...
    \subsubsection*{Advanced options}
      ...
      \begin{description}
          ...
          \item[store]\index{store}
          \index{store}
          \index{local storage|see{store}
          \index{session storage|see{store}
    ...
    \subsubsection*{Configuration settings}
       Use \BashCode|webquiz --edit-settings| to change these settings.
...
%   \end{heading}
  ...
  Vector Graphics} (SVG). This is done using \ctan{dvisvgm}. At first
  sight this is OK because \ctan{dvisvgm} is included in \TeXLive and
  \MiKTeX, however, \ctan{dvisvgm} uses \Ghostscript and this needs to
  be correctly configured and, as outlined in
  \href{http://dvisvgm.bplaced.net/FAQ}{FAQ}, \ctan{dvisvgm} needs to
  know where to find the \Ghostscript libraries. For example, to get
  \ctan{dvisvgm} to work on my system I needed to add the line
  \begin{bashcode}
